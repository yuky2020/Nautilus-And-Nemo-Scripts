...
hkey=$(zenity --entry --text "Enter string for encoding/decoding" --width 850 --entry-text "" --title "BIN to ASCII/DECIMAL/HEX/BASE 16 32 64 Encoder")
case $? in
0)
dataencode=$(
yy=$(echo "$hkey" | sed 's/ //g')
ykey=$(echo "$(echo "obase=16; ibase=2;"$yy"" | bc)";)
xkey=$(echo $ykey | xxd -r -p)
printf "%-20s\t" 'Normal:';
printf "\r"
echo "$xkey"; echo
printf "\r"
...
printf "%-20s\t" 'Base64:';
printf "\r"
python -c "import base64; print base64.b64encode('''$xkey''')"
printf "\r"
echo "---------------------------------------------------------------------------------------------------------------------------------------------"; echo
printf "\r"
printf "%-20s\t" 'Base32:';

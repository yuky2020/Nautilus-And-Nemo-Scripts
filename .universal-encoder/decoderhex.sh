...
hkey=$(zenity --entry --text "Enter string for encoding/decoding" --width 850 --entry-text "" --title "HEX to ASCII/DECIMAL/BIN/BASE 16 32 64 Encoder")
case $? in
0)
dataencode=$(
xkey=$(echo $hkey | xxd -r -p)
